...
def getimg(Tpath, Spath, x, y):
    ...
    try:
        ...
        pic = urllib.request.urlopen(req, timeout=15)
        f.write(pic.read())
        f.close()
    ...
    for x in range(lefttop[0], rightbottom[0]):
        ...
        with tqdm(range(lefttop[1], rightbottom[1])) as pbar:
            for y in pbar:
                ...
                #tilepath = "http://gac-geo.googlecnapps.club/maps/vt?lyrs=s&x=" + str(x) + "&y=" + str(y) + "&z=" + str(zoom)
...
if __name__ == "__main__":
    rootDir = "F:\\satellite"
    resultDir = "F:\\satellite\\result"
    # 瓦片地图的放大倍数
    zoom = 20
    #用于定义图片/区域编号

    #判断从哪个位置开始处理
    turn = find_maxpic.find_max_number_file('F:\\satellite\\result') + 1
